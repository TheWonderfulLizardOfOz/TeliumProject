...
def getNoOfPlayers():
    ...
    while True:
        try:
            ...
            print("Invalid input: please enter an integer value or a number more than 0.")

def setupPlayers(noOfPlayers):
    global players
    for i in range(noOfPlayers):
        key = "player" + " " + str(i + 1)
        players[key] = {"alive": True, "fuel": Fuel(500), "module": 1, "last module": 0}
...
def nextPlayer():
    global players, fuel, alive, module, currentPlayer, previousPlayer, previousPlayerCount, currentPlayerCount, last_module
    if len(players) == previousPlayerCount + 1:
        maxCounter = 0
    else:
        maxCounter = previousPlayerCount + 1
    counter = 0
    for player in players:
        if counter == maxCounter and players[player]["alive"] == True:
            currentPlayer = player
            fuel = players[player]["fuel"]
            alive = players[player]["alive"]
            module = players[player]["module"]
            last_module = players[player]["last module"]
            currentPlayerCount = counter
            break
        elif counter == maxCounter and players[player]["alive"] == False:
            maxCounter += 1
        counter += 1

def endPlayerTurn():
    global players, alive, fuel, module, currentPlayer, previousPlayer, previousPlayerCount, currentPlayerCount, last_module
    players[currentPlayer]["alive"] = alive
    players[currentPlayer]["fuel"] = fuel
    players[currentPlayer]["module"] = module
    players[currentPlayer]["last module"] = last_module
    previousPlayer = currentPlayer
    previousPlayerCount = currentPlayerCount
    
def checkAllAlive():
    global allAlive, players
    counter = 0
    dead = 0
    for player in players:
...
menu()
spawn_npcs()
print("Queen alien is located in module:", npcs["queen"])
print("Ventilation shafts are located in modules:", npcs["ventilation shafts"])
print("Information panels are located in modules:", npcs["information panel"])
print("Worker aliens are located in modules:", npcs["alien worker"])

while allAlive and not won:
    nextPlayer()
    print()
...
##    power_supply()
##    fuel.check_fuel()
    allAlive = checkAllAlive()
    if allAlive == True:
        allAlive = power.check_power()
    if won == False and allAlive == True:
        intuition()
        output_moves()
        get_action()
...
if won == True:
    print("You win! :)")
elif allAlive == False and power.check_power() == False:
